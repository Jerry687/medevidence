...
This preserves the previous milestone narrative. Its status and setup statements
are historical. Use the [current README](../../README.md), [runbook](../LOCAL_RUNBOOK.md)
and [delivery status](../DELIVERY_STATUS.md) for the current implementation.
...
## Current completion worktree

The local application now composes durable source collection, verified materials,
DeepSeek report generation, independent Qwen validation, checkpointed review and
human-approved export. FastAPI and read-only MCP have configured entry points;
Streamlit uses the research API. Missing approved CADEC assets remain a visible
source skip. This development delivery is not a held-out V1 release certification.

The exact original CADEC archive has been restored from CSIRO. Its missing project
manifest has a distinct reviewed recovery successor (ADR-043); the original
manifest and audit identities remain historical and are not fabricated.

The Owner selected `qwen3.8-max-0902` after its complete 36-case Development
comparison: 33/36 agreement with all original thresholds met. The runtime codec
replays the same 36 request/response pairs. DeepSeek's earlier failed calibration
attempts remain immutable history, not evidence for the Qwen profile. Raw evidence
is under project-root `.local/evidence/QWEN-COMPARISON-20260918-004/` and
`.local/evidence/V1-QWEN-DELIVERY-20260918/`.

See [local startup and remaining delivery gates](../LOCAL_RUNBOOK.md).
The following M1A approval record is historical, not the state of this worktree.

## Historical M1A approval record
...
- [Data-source semantics](../DATA_SOURCES.md)
- [Evaluation plan](../EVALUATION_PLAN.md)
- [Security and medical-safety policy](../SECURITY.md)
- [V1 traceability matrix](../TRACEABILITY_MATRIX.md)
- [Architecture decisions](../decisions/README.md)
- [Review, approval, and authorization records](../reviews)
- [Interview narrative](../INTERVIEW_NOTES.md)
