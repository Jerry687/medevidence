...
Manual release review should verify that Draft PR `#6` current head matches the
pushed feature ref, confirm both hosted checks remain green, inspect the evidence
and implementation commits in the PR history, and require separate Owner
authorization for merge/integration. Live PubMed remains a separate opt-in gate.
